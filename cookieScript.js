(async function () {
    ...
    const suspiciousPatterns = [ { pattern: /collect|plausible.io|googletagmanager|google-analytics|gtag|analytics|zoho|track|metrics|pageview|stat|trackpageview|clarity\.ms/i, category: "Analytics" }, { pattern: /facebook|meta|fbevents|linkedin|twitter|pinterest|tiktok|snap|reddit|quora|outbrain|taboola|sharethrough|matomo/i, category: "Marketing" }, { pattern: /optimizely|hubspot|marketo|pardot|salesforce|intercom|drift|zendesk|freshchat|tawk|livechat/i, category: "Personalization" } ];

    
    // Define the missing isSuspiciousResource function
    function isSuspiciousResource(url) {
        if (!url || typeof url !== 'string') return false;
        const cleanedUrl = url.replace(/^https?:\/\//, '').split('?')[0]; // Basic cleaning
        for (const { pattern } of suspiciousPatterns) {
            if (pattern.test(cleanedUrl)) {
                console.log(`Initial block: Suspicious resource detected - ${url}`);
                return true;
            }
        }
        return false;
    ...
    // Call initial blocking immediately
  ...
  async function restoreAllowedScripts(preferences) {
    ...
    // Add 'necessary' explicitly if not present, defaulting to true
    if (normalizedPrefs.necessary === undefined) {
        normalizedPrefs.necessary = true;
    }
     // Ensure ccpa sub-object exists for checks later, default DoNotShare to false if not present
    if (typeof normalizedPrefs.ccpa !== 'object' || normalizedPrefs.ccpa === null) {
        normalizedPrefs.ccpa = { donotshare: false };
    } else if (normalizedPrefs.ccpa.donotshare === undefined) {
         normalizedPrefs.ccpa.donotshare = false; // Default if ccpa object exists but key is missing
    }


    console.log("Scripts to potentially restore:", Object.keys(existing_Scripts).length);
    console.log("Using preferences for restore:", normalizedPrefs);
    ...
    for (const scriptId of scriptIdsToRestore) {
        ...
        if (!placeholder) {
            console.warn(`Placeholder for script ID ${scriptId} not found. Skipping.`);
            delete existing_Scripts[scriptId];
            continue;
        }

        // Determine if allowed: Check if *any* of the script's categories are true in normalizedPrefs
        // Also ensure 'Necessary' scripts are always allowed if the category is 'necessary'
        const isAllowed = scriptInfo.category.includes('necessary') || scriptInfo.category.some(cat => normalizedPrefs[cat] === true);


        console.log(`Script ID: ${scriptId}, Categories: [${scriptInfo.category.join(',')}], Allowed: ${isAllowed}`);

        if (isAllowed) {
            // Prevent duplicate loading if a script with the same src is already executing
            if (scriptInfo.src) {
                ...
                if (existingScript && existingScript !== placeholder) {
                    console.log(`Script with src ${scriptInfo.src} already loaded. Removing placeholder ${scriptId}.`);
            ...
            // Keep original category info if needed for debugging or other logic
            script.setAttribute("data-category", scriptInfo.category.join(','));
             // Mark as restored to potentially prevent re-blocking if observer runs too quickly
             script.setAttribute("data-consentbit-restored", "true");


            // --- Restore Attributes and Content ---
            if (scriptInfo.src) {
                script.src = scriptInfo.src;
                // Restore other original attributes for ALL external scripts
                Object.entries(scriptInfo.originalAttributes).forEach(([name, value]) => {
                    script.setAttribute(name, value);
                });

                // --- Consent API Integrations ---
                ...
                // Add more `else if` blocks here for other tools with specific consent APIs

                // --- End Consent API Integrations ---
                ...
                Object.entries(scriptInfo.originalAttributes).forEach(([name, value]) => {
                 ...
                 // Potentially check inline content for specific tool initializations if needed
                 // e.g., if (scriptInfo.content.includes('fbq(')) { setupFacebookConsent(null, normalizedPrefs); } // Call immediately if inline
            }
            ...
            if (placeholder.parentNode) {
                ...
                console.warn(`Placeholder parent node missing for ${scriptId}. Appending to head.`);
                document.head.appendChild(script); // Fallback
            }
            ...
            // Ensure it's still a placeholder
            if (placeholder.tagName !== 'SCRIPT' || placeholder.type !== 'text/plain') {
                console.warn(`Node for script ID ${scriptId} is not a placeholder!`);
            }
        }
    }
...
function setupGtagConsent(scriptElement, normalizedPrefs) {
    console.log("Setting up GA consent integration...");
    function updateGAConsent() {
        if (typeof window.gtag === "function") {
            console.log("Updating GA consent settings:", normalizedPrefs);
            window.gtag('consent', 'update', {
                'ad_storage': normalizedPrefs.marketing ? 'granted' : 'denied',
                'analytics_storage': normalizedPrefs.analytics ? 'granted' : 'denied',
                 // --- Google Consent Mode v2 Parameters ---
                 'ad_user_data': normalizedPrefs.marketing ? 'granted' : 'denied', // Related to sending user data for ads
                 'ad_personalization': normalizedPrefs.marketing ? 'granted' : 'denied', // Related to personalized advertising (e.g., remarketing)
                 'functionality_storage': normalizedPrefs.necessary ? 'granted' : 'denied', // Often necessary for site features
                  'personalization_storage': normalizedPrefs.personalization ? 'granted' : 'denied', // If using this category for UI/content personalization
                 'security_storage': normalizedPrefs.necessary ? 'granted' : 'denied', // Usually necessary for security features
            });
        } else {
            console.warn("window.gtag function not found yet for consent update.");
        }
    }
    // Attempt immediate update (if gtag already exists)
    updateGAConsent();
    // Set onload handler for the script element
    scriptElement.onload = () => {
        console.log(`GA script (${scriptElement.src}) loaded. Running post-load consent update.`);
        updateGAConsent();
    };
    scriptElement.onerror = () => console.error(`Failed to load GA script: ${scriptElement.src}`);
}
  ...
  function blockAllInitialRequests() {
    console.log("Activating initial network request blocking...");
    const originalFetch = window.fetch;
    window.fetch = function (...args) {
        const url = args[0] instanceof Request ? args[0].url : args[0];
        if (initialBlockingEnabled && isSuspiciousResource(url)) {
            console.log(`[Initial Block] Fetch blocked: ${url}`);
            return Promise.resolve(new Response(null, { status: 204, statusText: "Blocked by ConsentBit Initial Load" }));
        }
    ...
    const originalXHR = window.XMLHttpRequest;
      window.XMLHttpRequest = function() {
        ...
        const originalSend = xhr.send; // Need to potentially block send as well

        xhr.open = function(method, url) {
          // Store the URL to check it in send()
          this._consentBitUrl = url;
          // Don't block here yet, as open might be called before send
          return originalOpen.apply(xhr, arguments);
        };

        xhr.send = function() {
            if (initialBlockingEnabled && isSuspiciousResource(this._consentBitUrl)) {
                console.log(`[Initial Block] XHR blocked: ${this._consentBitUrl}`);
                // Abort the request or prevent sending.
                // Triggering an error event might be cleaner for the calling code.
                 this.dispatchEvent(new ProgressEvent('error', {
                    bubbles: false,
                    cancelable: false,
                 }));
                return; // Prevent the actual send
            }
    ...
    // Intercept Image src assignment
    const originalImage = window.Image;
    // We need to intercept the 'src' property assignment
    const ImageDescriptor = Object.getOwnPropertyDescriptor(HTMLImageElement.prototype, 'src');
    Object.defineProperty(HTMLImageElement.prototype, 'src', {
        configurable: true,
        enumerable: true,
        set: function(value) {
            if (initialBlockingEnabled && isSuspiciousResource(value)) {
                console.log(`[Initial Block] Image src blocked: ${value}`);
                // Set to a blank pixel or remove attribute? Setting to empty might be safer.
                // originalSetAttribute.call(this, 'src', ''); // Or use the original descriptor's setter if possible
                 if (ImageDescriptor && ImageDescriptor.set) {
                     ImageDescriptor.set.call(this, ''); // Call original setter with empty value
                 } else {
                     this.setAttribute('src', ''); // Fallback
                 }
            } else {
                if (ImageDescriptor && ImageDescriptor.set) {
                    ImageDescriptor.set.call(this, value); // Call original setter
                } else {
                     this.setAttribute('src', value); // Fallback
                }
            }
        },
        get: ImageDescriptor ? ImageDescriptor.get : function() { return this.getAttribute('src'); } // Use original getter or fallback
    });

     // Also handle direct setAttribute for src
     const originalSetAttribute = Element.prototype.setAttribute;
     Element.prototype.setAttribute = function(name, value) {
         if (this.tagName === 'IMG' && name.toLowerCase() === 'src' && initialBlockingEnabled && isSuspiciousResource(value)) {
             console.log(`[Initial Block] Image setAttribute(src) blocked: ${value}`);
             originalSetAttribute.call(this, name, ''); // Set to empty
         } else {
             originalSetAttribute.call(this, name, value);
         }
     };

      console.log("Initial network request blocking activated.");
  }
