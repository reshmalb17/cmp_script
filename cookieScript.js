(async function () {
  ...
  async function restoreAllowedScripts(preferences) {
    ...
    function updateToolConsent(script, normalizedPrefs) {
        // Define patterns for all tools
        const patterns = {
            gtag: /googletagmanager\.com\/gtag\/js/i,
            gtm: /googletagmanager\.com\/gtm\.js/i,
            optimizely: /optimizely\.com|cdn\.optimizely\.com/i,
            matomo: /matomo\.php|piwik\.php|matomo\.js/i,
            plausible: /plausible\.io|script\.plausible\.io/i,
            meta: /connect\.facebook\.net|fbevents\.js/i,
            clarity: /clarity\.ms|clarity\.js/i,
            hotjar: /hotjar\.com|hotjar\.js/i,
            linkedin: /snap\.licdn\.com|linkedin\.com\/insight/i,
            tiktok: /analytics\.tiktok\.com|bytedance\.com/i,
            pinterest: /pintrk\.js|pinterest\.com\/pin/i,
            twitter: /static\.ads-twitter\.com|analytics\.twitter\.com/i
        };

        // Google Analytics / GTM
        if (patterns.gtag.test(script.src) || patterns.gtm.test(script.src)) {
            gtag('consent', 'update', {
        ...
        // Optimizely
        if (patterns.optimizely.test(script.src)) {
            if (typeof optimizely === "object" && optimizely.setConsentStatus) {
                console.log("Updating Optimizely consent settings...");
                const optimizelyConsent = {
                    analytics: normalizedPrefs.analytics ? "granted" : "denied",
                    tracking: normalizedPrefs.marketing ? "granted" : "denied",
                    personalization: normalizedPrefs.marketing ? "granted" : "denied"
                };
                
                try {
                    optimizely.setConsentStatus(optimizelyConsent);
                    if (optimizely.setTargetingAttributes) {
                        optimizely.setTargetingAttributes({
                            consent_status: optimizelyConsent
                        });
                    }
                } catch (e) {
                    console.warn("Error updating Optimizely consent:", e);
                }
            }
        }

        // Matomo
        if (patterns.matomo.test(script.src)) {
            if (typeof _paq === "object") {
                ...
                try {
                    ...
                    if (normalizedPrefs.marketing) {
                        _paq.push(['setConsentGiven', 'marketing']);
                    }
                    if (normalizedPrefs.analytics) {
                        _paq.push(['setConsentGiven', 'analytics']);
                    }
                } catch (e) {
                    console.warn("Error updating Matomo consent:", e);
                }
            }
        }

        // Plausible
        if (patterns.plausible.test(script.src)) {
            if (typeof plausible === "function") {
                console.log("Updating Plausible consent settings...");
                try {
                    if (!normalizedPrefs.analytics) {
                        window.plausible_exclude = true;
                    } else {
                        delete window.plausible_exclude;
                    }
                    
                    plausible('consent', {
                        props: {
                            status: normalizedPrefs.analytics ? 'granted' : 'denied'
                        }
                    });
                } catch (e) {
                    console.warn("Error updating Plausible consent:", e);
                }
            }
        }

        // Meta Pixel (Enhanced)
        if (patterns.meta.test(script.src)) {
            if (typeof fbq === "function") {
                console.log("Updating Meta Pixel consent settings...");
                try {
                    fbq('consent', normalizedPrefs.marketing ? 'grant' : 'revoke');
                    ...
                    const metaConsent = {
                        ...
                        'functionality_storage': 'granted',
                        'security_storage': 'granted',
                        'personalization_storage': normalizedPrefs.marketing ? 'granted' : 'denied'
                    };

                    if (typeof fbq.gdpr === "function") {
                        fbq.gdpr(normalizedPrefs.marketing ? 1 : 0);
                    }

                    fbq('dataProcessingOptions', normalizedPrefs.marketing ? [] : ['LDU'], 0, 0);

                    if (typeof fbq.setUserProperties === "function") {
                        fbq.setUserProperties(1, {
                            allowAdvertiserFeatures: normalizedPrefs.marketing,
                            allowAdvertiserPersonalization: normalizedPrefs.marketing
                        });
                    }

                    fbq('trackSingle', 'consent_updated', metaConsent);
                } catch (e) {
                    console.warn("Error updating Meta Pixel consent:", e);
                }
            }
        }

        // Microsoft Clarity
        if (patterns.clarity.test(script.src)) {
            if (typeof clarity === "function") {
                console.log("Updating Microsoft Clarity consent settings...");
                try {
                    clarity('consent', normalizedPrefs.analytics);
                } catch (e) {
                    console.warn("Error updating Clarity consent:", e);
                }
            }
        }

        // Hotjar
        if (patterns.hotjar.test(script.src)) {
            if (typeof hj === "function") {
                console.log("Updating Hotjar consent settings...");
                try {
                    hj('consent', normalizedPrefs.analytics);
                } catch (e) {
                    console.warn("Error updating Hotjar consent:", e);
                }
            }
        }

        // LinkedIn Insight
        if (patterns.linkedin.test(script.src)) {
            if (typeof lintrk === "function") {
                console.log("Updating LinkedIn Insight consent settings...");
                try {
                    lintrk('track', { consent: normalizedPrefs.marketing ? 'granted' : 'denied' });
                } catch (e) {
                    console.warn("Error updating LinkedIn consent:", e);
                }
            }
        }

        // TikTok Pixel
        if (patterns.tiktok.test(script.src)) {
            if (typeof ttq === "function") {
                console.log("Updating TikTok Pixel consent settings...");
                try {
                    ttq.push(['track', 'SetConsent', {
                        'consent': normalizedPrefs.marketing ? 'granted' : 'denied'
                    }]);
                } catch (e) {
                    console.warn("Error updating TikTok consent:", e);
                }
            }
        }

        // Pinterest Tag
        if (patterns.pinterest.test(script.src)) {
            if (typeof pintrk === "function") {
                console.log("Updating Pinterest Tag consent settings...");
                try {
                    pintrk('consent', normalizedPrefs.marketing ? 'granted' : 'denied');
                } catch (e) {
                    console.warn("Error updating Pinterest consent:", e);
                }
            }
        }

        // Twitter Pixel
        if (patterns.twitter.test(script.src)) {
            if (typeof twq === "function") {
                console.log("Updating Twitter Pixel consent settings...");
                try {
                    twq('consent', normalizedPrefs.marketing ? 'grant' : 'revoke');
                } catch (e) {
                    console.warn("Error updating Twitter consent:", e);
                }
            }
        }
    }
        ...
        if (isAllowed) {
            ...
            if (scriptInfo.src) {
                ...
                script.onload = () => {
                    ...
                    updateToolConsent(script, normalizedPrefs);
    ...
    window.dispatchEvent(new CustomEvent('consentPreferencesUpdated', {
